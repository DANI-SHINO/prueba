...
from app.utils import verificar_token, generar_token, generar_reporte_con_plantilla, generar_reporte_populares, generar_reporte_prestados, obtener_datos_libro_openlibrary
...
def login():
    if current_user.is_authenticated:
        ...
        if current_user.rol == 'administrador':
            return redirect(url_for('main.catalogo'))  # O tu ruta real de admin
        elif current_user.rol == 'bibliotecario':
            return redirect(url_for('main.catalogo'))  # Ajusta si es diferente
...
def api_datos_libro(isbn):
    result = obtener_datos_libro_openlibrary(isbn)
...
def nuevo_libro():
    ...
    if isbn_param and request.method == 'GET':
        try:
            # ✅ LLAMA A LA FUNCIÓN LOCAL, NO A TU API
            data = obtener_datos_libro_openlibrary(isbn_param)
            ...
            flash(f"Error al obtener datos del libro: {e}", 'danger')
